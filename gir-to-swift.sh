...
gir2swift -p ${GIR_DIR}/GLib-2.0.gir "${GIR}" | sed -f ${Module}.sed > Sources/${Mod}/${Module}.swift
...
grep 'public protocol' Sources/${Mod}/${Module}.swift | cut -d' ' -f3 | cut -d: -f1 | sort -u | sed -e 's/^\(.*\)/public typealias _GModule_\1 = \1/' >> Sources/${Mod}/Swift${Mod}.swift
echo >> Sources/${Mod}/Swift${Mod}.swift ""
echo >> Sources/${Mod}/Swift${Mod}.swift ""
